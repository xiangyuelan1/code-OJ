...
export function AdminPromotionPage() {
  ...
  const [planForm, setPlanForm] = useState({ name: '', price: 0, duration: 30, unit: 'DAY', features: [''], isPopular: false, sortOrder: 0, studentQuota: 50, classQuota: 10, aiTokenQuota: 0 });
  ...
  const handleSavePlan = async () => {
    ...
    try {
      ...
      setEditPlanId(null);
      setPlanForm({ name: '', price: 0, duration: 30, unit: 'DAY', features: [''], isPopular: false, sortOrder: 0, studentQuota: 50, classQuota: 10, aiTokenQuota: 0 });
      fetchData();
    } catch (error: any) {
      ...
      {tab === 'pricing' && (
        <div className="space-y-6">
          <div className="flex justify-end">
            <button onClick={() => { setEditPlanId(null); setPlanForm({ name: '', price: 0, duration: 30, unit: 'DAY', features: [''], isPopular: false, sortOrder: 0, studentQuota: 50, classQuota: 10, aiTokenQuota: 0 }); setShowCreatePlan(true); }} className="flex items-center gap-2 px-4 py-2 bg-cyan-500 hover:bg-cyan-600 text-white rounded-lg transition-colors">
          ...
          <div className="bg-slate-800 rounded-xl p-6 max-w-lg w-full border border-slate-700" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div><label className="block text-sm text-slate-300 mb-1">班级名额 (0=无限制)</label><input type="number" value={planForm.classQuota} onChange={e => setPlanForm(f => ({ ...f, classQuota: Number(e.target.value) }))} min={0} className="w-full px-3 py-2 bg-slate-700 border border-slate-600 rounded-lg text-white focus:outline-none focus:border-cyan-500" placeholder="教师可创建的班级数量上限" /></div>
                <div><label className="block text-sm text-slate-300 mb-1">AI Token月度配额 (0=无限制)</label><input type="number" value={planForm.aiTokenQuota} onChange={e => setPlanForm(f => ({ ...f, aiTokenQuota: Number(e.target.value) }))} min={0} className="w-full px-3 py-2 bg-slate-700 border border-slate-600 rounded-lg text-white focus:outline-none focus:border-cyan-500" placeholder="每月AI Token使用上限" /></div>
              </div>
              <div>
